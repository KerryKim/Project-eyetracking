...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, split, imSize=(224,224), gridSize=(25,25)):
        ...
        if split == 'train':
            self.transformFace = transforms.Compose([SubtractMean(meanImg=self.faceMean), ToNumpy(), ToTensor()])
            self.transformEyeL = transforms.Compose([SubtractMean(meanImg=self.eyeLeftMean), ToNumpy(), ToTensor()])
            self.transformEyeR = transforms.Compose([SubtractMean(meanImg=self.eyeRightMean), ToNumpy(), ToTensor()])
